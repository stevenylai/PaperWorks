...
BEGIN{switchCount=SWITCH_NUMBER;seq=-1;switchAver=0;lastReading=0;switchDirection=0;eventCount=0;}
{
 if ($4==seq+1) {
	seq=$4;
 currentAver=$2;
...
END{
 if (eventCount-1==0)
	...
	printf("Events detected for sensor %d as %d, false rate %f\n", $1, eventCount-1, (eventCount-2)/(eventCount-1));}' SWITCH_NUMBER=${3} SWITCH_THRESHOLD=${4} REAL_EVENT=${5} EVENT_TYPE=${6}
