...
In order to develop a powerful pub/sub middleware, we need to provide a high-level powerful language to the application programmers. Such kind of language is primarily used to express complex events or event patterns. Unfortunately, unlike query-based middleware approach which has standard SQL language, event definition language should be more expressive in terms of the dynamics and relations among data. In this section, we take a look at the work that has already been made in this area. More specifically, we study the existing event-based systems from different aspects such as event definition, event evaluation and event operators / functions. These aspects are closely related to a successful event-based middleware.

\subsection{Event Definition}
Event definition may find its root in some very old works in distributed system since it is an important component in such systems. Some of the such works make use of tuple space to represent simple events. The idea is from Linda \cite{linda}. Linda is a parallel programming language that makes use of tuple space. Four basic operations are defined in Linda: out(), in(), read() and eval(). In \cite{linda}, the concept of actual and formal is described which are used by other later works such as \cite{lowlevelnaming} and \cite{tinylime}.
...
However, the events defined in these works may not be suitable for all kinds of pub/sub system. As a result, some later work defined events in a way which is more comprehensive than tuple space. For example, Meghdoot \cite{meghdoot} defines event as a conjunction of pairs, where each pair consists of an attribute and a value. A subscription is a collection of predicates each of which is a triple consisting of an attribute, a value and a relational operator.
...
The Yeast specification language has the following format: event\_pattern do action. In Yeast, events are categorized as time events, object events and compound events. Each kind of event has a set of descriptors. Time events have 'in' and 'within' which specifies the event is after of before a time. Object descriptors have the following format: obj\_class obj\_name obj\_attr relational\_test. In addition, two special relation tests 'changed' and 'unchanged' are defined. Compound event descriptors are combination of time and object events with 'then', 'and' and 'or'. An example of a Yeast script is shown in Listing \ref{lst:yeast1}.
...
To demonstrate the interactive usage of yeast, on Line \ref{lst:yeast3:eventStart} - Line \ref{lst:yeast3:eventEnd} of Listing \ref{lst:yeast3}, one developer first defines an event that is used to monitor the debugging status of a source code file. Then once the file has been debugged, on Line \ref{lst:yeast3:announce}, the other developer announces the new status of the file and the corresponding developers will be notified.
...
A popular approach for event evaluation in the field of active database is rule-based approach. As one of the most frequently referenced work, GEM \cite{gem} is an interpreted generalized event monitoring language. It allows abstract events to be specified in terms of combinations of lower-level events. GEM is a declarative rule-based language in which various temporal constraints can be specified for event composition. Each GEM script is constructed from some basic GEM primitives. A few of them is defined in Listing \ref{lst:GEMPrimitives}
